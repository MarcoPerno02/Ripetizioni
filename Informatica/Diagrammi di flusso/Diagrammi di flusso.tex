...
\node (output) [io, right=2cm of decision] {Scrivi "Positivo"};
\node (stop) [startstop, below=2cm of decision] {Fine};

\draw [arrow] (start) -- (input);
\draw [arrow] (input) -- (decision);
\draw [arrow] (decision) -- node[anchor=south] {Vero} (output);
...
$<$ & Minore di & $x < 10$ \\
$>$ & Maggiore di & $x > 0$ \\
$\leq$ o $>=$ & Minore o uguale & $x \leq 100$ \\
$\geq$ o $<=$ & Maggiore o uguale & $x \geq 18$ \\
\hline
\end{tabular}
\end{center}
...
\textbf{Operatore} & \textbf{Significato} & \textbf{Esempio} \\
\hline
AND o \&\& (E) & Entrambe vere & $(x > 0)$ AND $(x < 10)$ \\
OR o $||$ (O) & Almeno una vera & $(x < 0)$ OR $(x > 100)$ \\
NOT o $!$ (NON) & Negazione & NOT $(x = 0)$ \\
\hline
\end{tabular}
\end{center}
...
\node (start) [startstop] {Inizio};
\node (input) [io, below of=start] {Leggi numero};
\node (calc) [process, below of=input] {$resto \leftarrow numero \% 2$};
