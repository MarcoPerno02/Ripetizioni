...
\begin{circuitikz}[scale=1.3]
    ...
    % Freccia maglia
    \draw[->, very thick, green!50!black] (2.5,1.5) arc (180:540:0.8);
    \node[green!50!black] at (2.5,0.5) {senso maglia};
\end{circuitikz}
\end{center}
...
\begin{circuitikz}[scale=1.3]
    ...
    \draw (0,0) to[isource, invert, l_=$I_g{=}2\,\mathrm{A}$, i>^=$I_{tot}$] (0,4)
          to[short] (1,4)
          to[R=$R_1{=}40\,\Omega$, v=$V_1$, i>^=$I_{tot}$] (3,4);

    % Nodo A
    \node[circ, label=above:A] at (3,4) {};

    % Ramo sinistro
    \draw (3,4) to[short, i=$I_2$] (3,3.7)
          to[R=$R_2{=}80\,\Omega$, v=$V_{AB}$, i>^=$I_2$] (3,2)
          to[short] (3,1.7)
          to[R=$R_4{=}60\,\Omega$, v=$V_{AB}$, i>^=$I_2$] (3,0.3)
          to[short] (3,0);

    % Ramo destro
    \draw (3,4) to[short, i=$I_3$] (7,4)
          to[R=$R_3{=}120\,\Omega$, v=$V_{AB}$, i>^=$I_3$] (7,2)
          to[short] (7,1.7)
          to[R=$R_5{=}90\,\Omega$, v=$V_{AB}$, i>^=$I_3$] (7,0)
          to[short] (3,0);

    % Ponte centrale (cortocircuito)
    \draw (3,2) to[short, i=$I_6$] (7,2);

    % Nodo B
    \node[circ, label=below:B] at (3,0) {};

    % Resistenza di chiusura inferiore
    \draw (3,0) to[R=$R_7{=}50\,\Omega$, v=$V_7$, i>^=$I_{tot}$] (1,0)
          to[short] (0,0);
